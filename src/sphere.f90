...
!
! requires NCAR's SPHEREPACK 3.0
...
! implicit typing, everything in module is public.
...
! recomputed.
      ...
      integer, save, private :: saved_nlon_reg = -1  ! initialize to nonsense
      integer, save, private :: saved_nlat_reg = -1  ! initialize to nonsense
      integer, save, private :: saved_nlon_gau = -1  ! initialize to nonsense
      integer, save, private :: saved_nlat_gau = -1  ! initialize to nonsense
      logical, save, private :: lfrst_grdtospec_reg=.TRUE.
      logical, save, private :: lfrst_spectogrd_reg=.TRUE.
      logical, save, private :: lfrst_vgrdtospec_reg=.TRUE.
      logical, save, private :: lfrst_vspectogrd_reg=.TRUE.
      logical, save, private :: lfrst_grdtospec_gau=.TRUE.
      logical, save, private :: lfrst_spectogrd_gau=.TRUE.
      logical, save, private :: lfrst_vgrdtospec_gau=.TRUE.
      logical, save, private :: lfrst_vspectogrd_gau=.TRUE.

! work arrays are allocated in module subroutines when above
! logical variables are .TRUE., or when nlon or nlat change.

      real, dimension(:), allocatable, save, private :: wshaes,wshses,wvhaes,wvhses
      real, dimension(:), allocatable, save, private :: wshags,wshsgs,wvhags,wvhsgs
...
!
      ...
      real*8, dimension(:), allocatable :: theta,wts
      real*8, dimension(:), allocatable :: workdp
...
! (dataspec).
      ...
      real*8, dimension(2*(size(datagrid,2)+1)) :: dwork
...
! from size of spectral arrays.
      ...
      if (nlon .ne. saved_nlon_reg .or. nlat .ne. saved_nlat_reg) then
...
! rsphere is the radius of the sphere in meters.
      ...
      real*8, dimension(2*(size(ugrid,2)+1)) :: dwork
...
! (dataspec).
      ...
      real, dimension((4*size(datagrid,1)+2)*size(datagrid,2)) :: work
      real*8, dimension((3*size(datagrid,2)*(size(datagrid,2)+3)+2)/2) :: dwork
      real, dimension(size(datagrid,2),size(datagrid,1)) :: temp
      real, dimension(size(datagrid,2),size(datagrid,2)) :: a,b
...
! from size of spectral arrays.
      ...
      nlon = size(datagrid,1)
      nlat = size(datagrid,2)
      if (nlon .ne. saved_nlon .or. nlat .ne. saved_nlat) then
...
! rsphere is the radius of the sphere in meters.
      ...
      real, dimension((4*size(ugrid,1)+2)*size(ugrid,2)) :: work
      real*8, dimension((3*size(ugrid,2)*(size(ugrid,2)+3)+2)/2) :: dwork
      real, dimension(size(ugrid,2),size(ugrid,1)) :: v,w
      real, dimension(size(ugrid,2),size(ugrid,2)) :: br,bi,cr,ci,a,b
      real, dimension(size(ugrid,2)) :: sqnn
